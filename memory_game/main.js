...
// Gets the number of images from the object stored in session
const getNumberOfImages = () => parseInt(sessionStorage.getItem("num")) || 24;

// Gets the name of the player from the object stored in session
const getPlayerName = () => sessionStorage.getItem("name")|| "";
...
$(document).ready( () => {
        ...
        $(a).click( evt => {
            ...
            if (isFirst) { 
                ...
                if (lastA.attr("id") === a.attr("id")) {
                    ...
                    if ( scores.matches == images.length) {
                        setTimeout( () => { 
                            //const percent = Math.floor((correctSelection / selection) * 100);
                            const percent = scores.calcScore();
    ...
    $("#save_settings").click( () => {
        ...
        sessionStorage.setItem("num", settings.numImages);
